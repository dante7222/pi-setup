...
`delegate_loop` requires a clean parent checkout with no active Git operation or submodules, creates one coding worktree, and runs up to three implementation attempts by default. Pi shows the exact validation commands and requires interactive confirmation before creation, then repeats the clean-parent preflight immediately before creating the worktree. After each attempt, those commands run in a dedicated process group with per-command timeouts, bounded logs, and process-tree termination. Checks must pass before fresh read-only reviewers, launched with candidate-controlled project files and context resources disabled, inspect an immutable base diff and recorded evidence. Reviewers end with a fail-closed `PASS`, `REPAIR`, or `BLOCKED` verdict; repairs start another fresh coding worker in the same worktree. Repeated candidate filesystem state, a worker-created submodule or embedded repository, a blocker, worker failure, or the attempt limit stops the loop. Loop state and per-attempt prompts, outputs, checks, evidence, reviews, and transitions are stored with the canonical worker job. Accepted output records the reviewed Git tree; `/delegate-apply` refuses rejected loops, any worktree changed after acceptance, or a destination HEAD that moved after review. Applying still requires a separate explicit request and confirmation, and workers are instructed not to push or merge.
