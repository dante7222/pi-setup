...
- `/delegate-recover [worker-id]` — reconcile interrupted provisioning, evaluator worktrees, terminal state, stale destination locks, and apply journals after the authenticated orchestrator has released ownership or exited.
- `/delegate-prune` — after confirmation, claim and delete old terminal research-job directories outside both the newest 100 jobs and the 30-day retention window; coding, live, unknown, actively recovering, recovery-required, and all recorded loop-reviewer or published-acceptance evidence is retained.
...
Coding worktree folders combine a readable task slug with the first 12 hexadecimal characters of the worker UUID, for example `tree-sitter-bash-permissions-4e9a04efc879`. Git branches use the full `pi-agent/<batch-uuid>/<worker-uuid>` identity. Interrupted provisioning is recovered only from the exact returned Supacode ID/path and its durable provenance token; branch-name inference is forbidden.
...
`delegate_loop` requires a clean parent checkout with no active Git operation or submodules, creates one coding worktree, and runs up to three implementation attempts by default. Pi shows terminal-escaped validation commands and requires interactive confirmation before creation, then repeats the clean-parent preflight immediately before creating the worktree. After each implementation process fully exits, the loop snapshots the filesystem twice, creates a synthetic commit plus persistent candidate ref, and binds the whole attempt to that immutable tree. Every check runs in a fresh detached Git checkout with a dedicated process group, timeout, bounded log, authenticated process/exit records, and before/after tree attestation. Each concurrent read-only reviewer receives another independent detached checkout of the same commit; its runner, exit, and terminal evidence are authenticated, and persistent check or reviewer mutation invalidates that gate. Reviewers end with a fail-closed `PASS`, `REPAIR`, or `BLOCKED` verdict; repairs start another fresh coding worker in the source worktree. Final acceptance revalidates the candidate ref, source HEAD, branch, and filesystem, then hashes the immutable policy, delegation base/destination, exact patch, checks, reviews, current iteration, and all prior candidate history into a gate manifest before the exclusive accept decision. Recovery and apply verify that complete evidence chain. A candidate seen by an earlier attempt can never be retried into acceptance. Gitlinks, blockers, worker failure, or the attempt limit also stop the loop. Applying still requires a separate explicit request and confirmation.
