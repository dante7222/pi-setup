...
Worker tabs use the parent Pi session name (or project directory) plus a short batch ID, for example `agents: auth-review [a7f3]`. Each tab has an inert anchor surface, and every worker—including the first—runs in its own separately monitored split. Research workers continue to split breadth-first across the batch tab. Coding workers are not tiled together because each tab belongs to its assigned worktree.
...
Workers inherit the parent model and thinking level unless overridden, and they also inherit the parent's YOLO mode. They do not inherit the parent conversation, so delegated tasks must be self-contained. Parent-facing worker summaries are capped at 50 KiB across the complete result; operational IDs and raw Git status remain in structured details, while full worker answers and logs remain available through artifact paths. Worker-session usage is aggregated into the parent tool result. Before every worktree, tab, or surface creation, versioned lifecycle intent with predetermined resource IDs is durably written. Completion, timeout, cancellation, runner failure, and recovery compete for one exclusive terminal claim. Workers run Pi in non-interactive print mode. Normal completion is not returned until the authenticated wrapper-exit sentinel exists and recorded process identities are absent; forced terminal states require independently verified termination. Individual surfaces are monitored independently; a confirmed manual close aborts the parent turn and starts verified termination. Timeouts and explicit cancellation close the exact surface, verify its disappearance, then verify or safely signal only matching recorded process groups. Indeterminate termination remains `recovery_required` instead of being reported as cleanup success. Worker tabs stay open by default; `keepOpen: false` closes them after capture. Each worker defaults to a 15-minute timeout.
